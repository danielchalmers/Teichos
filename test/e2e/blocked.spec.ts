...
test('go back restores the last allowed url', async ({
  ...
  await page.goto(`${extensionPage('tabs/blocked.html')}?url=${encodeURIComponent(blockedUrl)}`);
...
test('opens settings from the blocked page', async ({ context, extensionPage, page }) => {
  await page.goto(
    `${extensionPage('tabs/blocked.html')}?url=${encodeURIComponent('https://blocked.example.invalid')}`
  );
